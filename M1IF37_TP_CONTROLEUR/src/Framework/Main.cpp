...
static void sKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if(action == GLFW_RELEASE){
       switch(key) {
            case GLFW_KEY_B:	// Reset walk to none
			    if (app) {settings.walkControl = 0;}
				break;
			case GLFW_KEY_N:
				if (app) {settings.walkControl = 0;}
				break;
            case GLFW_KEY_X:	// Reset jump
				if (app) {settings.jumpControl = false;}
				break;
		}
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_SPACE: // Launch a ball
			...
			break;
        case GLFW_KEY_B: // gauche
            if(app) {settings.walkControl = -1;}
            break;
        case GLFW_KEY_N: // droite
            if(app) {settings.walkControl = 1;}
            break;
        case GLFW_KEY_X: // SAUT
            if(app) {settings.jumpControl = true;}
            break;
...
settings.walkControl = 0;
settings.jumpControl = false;
	delete app;
	app = new Application;
}
...
static void sSimulate() {
	...
	app->Step(&settings);
	app->DrawTitle("Controles : Esc=quitter, fleches ou souris+droit=deplacer, souris+gauche=selectionner+deplacer, haut/bas=zoom, R=reset, space=balle, P=pause, X=sauter, B=deplacer gauche, N= deplacer droite");
